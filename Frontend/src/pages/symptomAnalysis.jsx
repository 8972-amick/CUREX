    ...
    <div className="min-h-screen bg-gradient-to-br from-emerald-50 via-white to-teal-50 flex items-center justify-center px-4 py-12">
      <div className="bg-white shadow-xl rounded-2xl p-8 max-w-2xl w-full border border-emerald-100/50">
        ...
        <button
          onClick={handleAnalyze}
          disabled={loading}
          className="mt-4 w-full bg-emerald-500 text-white py-3 rounded-lg font-semibold hover:bg-emerald-600 transition disabled:opacity-60"
        >
          {loading ? "Analyzing..." : "Analyze Symptoms"}
        </button>
        ...
        {result && (
          <div className="mt-6 p-4 rounded-xl bg-amber-50 border border-amber-200">
            ...
            <p className="text-amber-800 font-medium text-sm mb-3">
              ⚠️ {result.disclaimer || "This is not a medical diagnosis."}
            </p>
            ...
            {result.possibleConditions?.length > 0 ? (
                  ...
                  {result.possibleConditions.map((c, i) => (
                    ...
                    >
                      ...
                      <div className="text-xs text-gray-500 mt-1">
                        {c.matchCount} / {c.totalSymptoms} symptoms matched
                      </div>
              ...
              <p className="text-gray-700">
                {result.advice ||
                  "No matching symptoms found. Please consult a doctor."}
              </p>
            )}
          </div>
        )}
